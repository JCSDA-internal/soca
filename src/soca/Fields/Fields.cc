...
namespace soca {
  ...
  {
    soca_field_create_f90(ftn_, geom_->toFortran(), vars_);
  }
  // -----------------------------------------------------------------------------
    ...
    : geom_(other.geom_), vars_(other.vars_), time_(other.time_)
  {
    soca_field_create_f90(ftn_, geom_->toFortran(), vars_);
    if (copy) {
      soca_field_copy_f90(ftn_, other.ftn_);
    } else {
      soca_field_zero_f90(ftn_);
    }
  }
  // -----------------------------------------------------------------------------
  ...
  {
    soca_field_create_f90(ftn_, geom_->toFortran(), vars_);
    soca_field_change_resol_f90(ftn_, other.ftn_);
  }
  // -----------------------------------------------------------------------------
  ...
  // -----------------------------------------------------------------------------
  Fields::~Fields() {
    soca_field_delete_f90(ftn_);
  }
  // -----------------------------------------------------------------------------
  Fields & Fields::operator=(const Fields & rhs) {
    soca_field_copy_f90(ftn_, rhs.ftn_);
    time_ = rhs.time_;
    return *this;
  }
  // -----------------------------------------------------------------------------
  Fields & Fields::operator+=(const Fields & rhs) {
    soca_field_self_add_f90(ftn_, rhs.ftn_);
    return *this;
  }
  // -----------------------------------------------------------------------------
  Fields & Fields::operator-=(const Fields & rhs) {
    soca_field_self_sub_f90(ftn_, rhs.ftn_);
    return *this;
  }
  // -----------------------------------------------------------------------------
  Fields & Fields::operator*=(const double & zz) {
    soca_field_self_mul_f90(ftn_, zz);
    return *this;
  }
  // -----------------------------------------------------------------------------
  void Fields::zero() {
  ...
  void Fields::axpy(const double & zz, const Fields & rhs) {
    soca_field_axpy_f90(ftn_, zz, rhs.ftn_);
  }
  // -----------------------------------------------------------------------------
  double Fields::dot_product_with(const Fields & fld2) const {
    double zz;
    soca_field_dot_prod_f90(ftn_, fld2.ftn_, zz);
    return zz;
  }
  // -----------------------------------------------------------------------------
  void Fields::schur_product_with(const Fields & dx) {
    soca_field_self_schur_f90(ftn_, dx.ftn_);
  }
  // -----------------------------------------------------------------------------
  void Fields::random() {
    soca_field_random_f90(ftn_);
  }
  // -----------------------------------------------------------------------------
  void Fields::getValues(const ufo::Locations & locs,
                         const oops::Variables & vars,
                         ufo::GeoVaLs & gom) const {
    soca_field_interp_nl_f90(ftn_, locs.toFortran(), vars,
                             gom.toFortran());
  }
  ...
  void Fields::getValues(const ufo::Locations & locs,
    ...
    soca_field_interp_nl_traj_f90(ftn_, locs.toFortran(), vars,
  ...
  void Fields::getValuesTL(const ufo::Locations & locs,
                           ...
                           ufo::GeoVaLs & gom,
                           const GetValuesTraj & traj) const {
    soca_field_interp_tl_f90(ftn_, locs.toFortran(), vars,
                             gom.toFortran(), traj.toFortran());
  }
  // -----------------------------------------------------------------------------
  void Fields::getValuesAD(const ufo::Locations & locs,
    ...
    soca_field_interp_ad_f90(ftn_, locs.toFortran(), vars,
                             gom.toFortran(), traj.toFortran());
  }
  // -----------------------------------------------------------------------------
  oops::GridPoint Fields::getPoint(const soca::GeometryIterator & iter) {
    ...
    soca_field_getpoint_f90(ftn_, iter.toFortran(), values[0],
                            values.size());
  ...
  void Fields::setPoint(const soca::GeometryIterator & iter,
    ...
    soca_field_setpoint_f90(ftn_, iter.toFortran(), vals[0], vals.size());
  }
  // -----------------------------------------------------------------------------
  ...
  void Fields::add(const Fields & rhs) {
    soca_field_add_incr_f90(ftn_, rhs.ftn_);
  }
  // -----------------------------------------------------------------------------
  void Fields::diff(const Fields & x1, const Fields & x2) {
    soca_field_diff_incr_f90(ftn_, x1.ftn_, x2.ftn_);
  }
  // -----------------------------------------------------------------------------
  void Fields::ug_coord(oops::UnstructuredGrid & ug) const {
    soca_field_ug_coord_f90(ftn_, ug.toFortran());
  }
  // -----------------------------------------------------------------------------
  void Fields::field_to_ug(oops::UnstructuredGrid & ug,
                           const int & its) const {
    soca_field_field_to_ug_f90(ftn_, ug.toFortran(), its);
  }
  // -----------------------------------------------------------------------------
  void Fields::field_from_ug(const oops::UnstructuredGrid & ug,
                             const int & its) {
    soca_field_field_from_ug_f90(ftn_, ug.toFortran(), its);
  }
  // -----------------------------------------------------------------------------
  ...
  void Fields::write(const eckit::Configuration & config) const {
    ...
    soca_field_write_file_f90(ftn_, &conf, &dtp);
  }
  // -----------------------------------------------------------------------------
  double Fields::norm() const {
    double zz = 0.0;
    soca_field_rms_f90(ftn_, zz);
    return zz;
  }
  // -----------------------------------------------------------------------------
  void Fields::print(std::ostream & os) const {
    ...
    int ncat = -1;
    int nf = -1;
    soca_field_sizes_f90(ftn_, nx, ny, nzo, nzi, ncat, nf);
    std::vector<double> zstat(3*nf);
    soca_field_gpnorm_f90(ftn_, nf, zstat[0]);
