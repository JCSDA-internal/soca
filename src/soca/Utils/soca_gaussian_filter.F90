...
subroutine soca_gaussian_filter_init(self, geom, hz_scales)
  ...
  ! n_min = ceil( 3/2 * sigma^2 / grid_size^2 )
  r_tmp = 0.0
  do j=geom%jsd,geom%jed
    do i=geom%isd,geom%ied
      if (.not. mask(i,j)) cycle
      r_tmp(i,j) = 3.0/2.0 * hz_scales(i,j)**2 / min(dx(i,j), dy(i,j)) **2
    end do
  end do
  ...
  do i = self%geom%isd, self%geom%ied
    do j = self%geom%jsd, self%geom%jed
      ...
      ! calculate the weights, first assuming we have 2 valid neighbors
  ...
  do i = self%geom%isd, self%geom%ied
    do j = self%geom%jsd, self%geom%jed
      ...
      ! calculate the weights, first assuming we have 2 valid neighbors
      self%weights_y(-1,i,j) = hz_scales(i,j)**2 / (2.0 * self%n_iter * dy(i,j)**2)
