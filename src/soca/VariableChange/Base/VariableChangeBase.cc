...
VariableChangeFactory::VariableChangeFactory(const std::string & name) {
  if (getMakers().find(name) != getMakers().end()) {
    oops::Log::error() << name << " already registered in soca::VariableChangeFactory."
                       << std::endl;
...
VariableChangeBase * VariableChangeFactory::create(const Geometry & geom,
  ...
  typename std::map<std::string, VariableChangeFactory*>::iterator jloc = getMakers().find(id);

  if (jloc == getMakers().end()) {
    oops::Log::error() << id << " does not exist in soca::VariableChangeFactory." << std::endl;
...
VariableChangeFactory::createParameters(const std::string &name) {
  typename std::map<std::string, VariableChangeFactory*>::iterator it =
      getMakers().find(name);
  if (it == getMakers().end()) {
    throw std::runtime_error(name + " does not exist in soca::VariableChangeFactory");
  }
