...
!
  ...
  public :: soca_models_init, soca_models_end
...
!> \brief Initialize all defined exchange grids and all boundary maps
  subroutine soca_models_init(Ocean, Ocean_state, Ice)
...
!-----------------------------------------------------------------------
    if ( do_endpoint_chksum ) then
      if (Atm%pe) then
        call mpp_set_current_pelist(Atm%pelist)
        call atmos_ice_land_chksum('soca_models_init+', 0, Atm, Land, Ice, &
                     Land_ice_atmos_boundary, Atmos_ice_boundary, Atmos_land_boundary)
      endif
      if (Ice%slow_ice_PE) then
        call mpp_set_current_pelist(Ice%slow_pelist)
        call slow_ice_chksum('soca_models_init+', 0, Ice, Ocean_ice_boundary)
      endif
      if (Ocean%is_ocean_pe) then
        call mpp_set_current_pelist(Ocean%pelist)
        call ocean_chksum('soca_models_init+', 0, Ocean, Ice_ocean_boundary)
      endif
    endif
...
!-----------------------------------------------------------------------
    ...
    if ( do_endpoint_chksum ) then
      if (Atm%pe) then
        call mpp_set_current_pelist(Atm%pelist)
        call atmos_ice_land_chksum('soca_models_end', 0, Atm, Land, Ice, &
                 Land_ice_atmos_boundary, Atmos_ice_boundary, Atmos_land_boundary)
      endif
      if (Ice%slow_ice_PE) then
        call mpp_set_current_pelist(Ice%slow_pelist)
        call slow_ice_chksum('soca_models_end', 0, Ice, Ocean_ice_boundary)
      endif
      if (Ocean%is_ocean_pe) then
        call mpp_set_current_pelist(Ocean%pelist)
        call ocean_chksum('soca_models_end', 0, Ocean, Ice_ocean_boundary)
      endif
    endif
    call mpp_set_current_pelist()
