...
!> Fortran module handling observations for the model

module c_soca_obs_data
...
contains
  ...
  subroutine soca_obs_setup(c_key_self, c_conf) bind(c,name='soca_obsdb_setup_f90')
    implicit none
    ...
    write(record,*)'soca_obs_setup: file in =',trim(fin)
    call fckit_log%info(record)

    fout = config_get_string(c_conf,max_string, "ObsData.ObsDataOut.obsfile")
    write(record,*)'soca_obs_setup: file out=',trim(fout)
    call fckit_log%info(record)
  ...
  end subroutine soca_obs_setup

  ! ------------------------------------------------------------------------------

  subroutine soca_obs_delete(c_key_self) bind(c,name='soca_obsdb_delete_f90')
    implicit none
    integer(c_int), intent(inout) :: c_key_self
    type(obs_data), pointer :: self
  ...
  end subroutine soca_obs_delete

  ! ------------------------------------------------------------------------------

  subroutine soca_obs_get(c_key_self, lreq, c_req, lcol, c_col, c_key_ovec) bind(c,name='soca_obsdb_get_f90')
    implicit none
    ...
    call obs_data_registry%get(c_key_self, self)
    call soca_obs_vect_registry%get(c_key_ovec,ovec)
    call c_f_string(c_req, req)
    call c_f_string(c_col, col)

    call obs_get(self, trim(req), trim(col), ovec)

  end subroutine soca_obs_get

  ! ------------------------------------------------------------------------------

  subroutine soca_obs_put(c_key_self, lreq, c_req, lcol, c_col, c_key_ovec) bind(c,name='soca_obsdb_put_f90')
    implicit none
    integer(c_int), intent(in) :: c_key_self
    integer(c_int), intent(in) :: lreq, lcol
    character(kind=c_char,len=1), intent(in) :: c_req(lreq+1), c_col(lcol+1)
  ...
  end subroutine soca_obs_put

  ! ------------------------------------------------------------------------------

end module c_soca_obs_data
